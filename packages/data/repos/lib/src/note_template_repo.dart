...
class NoteTemplateRepo {
  ...
  Future<void> createNewNoteTemplate(
    ...
    List<(List<String> frontFields, List<String> backFields)> cardTemplates,
      ...
      var (newNoteTemplate, fields) = await noteTemplateApi.createNoteTemplate(name, noteFields);
      ...
      for (var cardTemplate in cardTemplates) {
        ...
        var mappedFront = frontFields.map((e) => (api.CardSide.front, noteFieldsIndexMap[e]!)).toList();
        var mappedBack = backFields.map((e) => (api.CardSide.back, noteFieldsIndexMap[e]!)).toList();
